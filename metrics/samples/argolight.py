...
class ArgolightReporter(Reporter):
    ...
    def __init__(self):
        image_report_to_func = {'spots': self.full_report_spots,
    ...
    def plot_homogeneity_map(self, image):
        ...
        for ch in range(nr_channels):
            data = table.slice([col_names.index(w_col) for w_col in wanted_columns],
                               table.getWhereList(condition=f'channel=={ch}', variables={}, start=0, stop=row_count,
                                                  step=0))
            ...
            x_positions = np.array(
                [val for col in data.columns for val in col.values if col.name == 'x_weighted_centroid'])
            y_positions = np.array(
                [val for col in data.columns for val in col.values if col.name == 'y_weighted_centroid'])
            ...
            ax[ch + nr_channels].imshow(np.flipud(interpolated_intgr_int),
            ...
            ax[ch + 2 * nr_channels].imshow(np.flipud(interpolated_max_int),
                                            extent=(0, x_dim, y_dim, 0),
                                            origin='lower',
                                            cmap=cm.hot,
        ...
        for ch_A in range(nr_channels):
            ...
            for ch_B in [i for i in range(nr_channels) if i != ch_A]:
                data = table.slice(list(range(len(col_names))),
                                   table.getWhereList(condition=f'(channel_A=={ch_A})&(channel_B=={ch_B})',
                                                      variables={},
                                                      start=0, stop=row_count, step=0))
                labels_map = np.array(
                    [val for col in data.columns for val in col.values if col.name == 'ch_A_roi_labels'])
                ...
                filtered_positions = positions_map[
                                     np.intersect1d(mask_labels, labels_map, assume_unique=True, return_indices=True)[
                                         1], :]
